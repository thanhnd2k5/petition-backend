...
// Middleware kiểm tra trạng thái yêu cầu in
export function verifyPrintJobStatus(req, res, next) {
    try {
        const { printJob } = req

        // Kiểm tra xem yêu cầu in có thể được cập nhật không
        if (printJob.status !== 'pending') {
            return abort(400, 'Không thể cập nhật yêu cầu in đã được xử lý')
        }
